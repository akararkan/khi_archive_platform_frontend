  ...
  useEffect(() => {
    ...
    apiClient
      ...
      .catch((err) => {
        ...
        // Degrade gracefully instead of a hard failure: a field the backend
        // hasn't migrated to the authenticated proxy yet may still be a
        // plain, CORS-open URL that a direct <img>/<audio>/<video> src can
        // load even though a cross-origin blob fetch can't read its bytes.
        // If it's genuinely unreachable, the caller's own onError/player
        // error handling takes over from here exactly as it did before.
        setState({ url: resolveMediaUrl(path), loading: false })
      })
