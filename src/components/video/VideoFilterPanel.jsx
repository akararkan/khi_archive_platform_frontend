// Inline expandable filter panel for the video table inside the
// project detail page. Composed entirely from the shared list-filter
// primitives so the visual language matches Audio, Image, Person, and
// Category.
//
// Sections, top-down: Classification, Language, Place / Event,
// Technical, Codecs / Audio, Versioning, People, Discovery, Catalog,
// Rights, Description / Note, Lifecycle, Activity. A user looking for
// "1080p H.264 clips" finds Technical + Codecs at a glance; someone
// after "documentaries directed by X" reaches People + Classification.
...
export function VideoFilterPanel({
  ...
  return (
    <FilterPanel
      open={open}
      title="Filter videos"
      description="All filters apply across the videos in this project. Combine sections to drill down — every chip below the panel can be cleared individually."
      count={activeCount}
      onClear={isAnyActive ? onClear : null}
      onClose={onClose}
    >
      {/* Classification — what kind of piece this is */}
      <FilterSection icon={VideoIcon} label="Classification" columns={3}>
        <FilterField label="Status">{txt('videoStatus', '')}</FilterField>
        <FilterField label="Version">{txt('videoVersion', 'RAW / MASTER')}</FilterField>
      ...
      <FilterSection icon={Languages} label="Language" columns={3}>
        ...
        <FilterField label="Subtitle" hint="contains">{txt('subtitle', 'language…')}</FilterField>
      </FilterSection>

      {/* Place / Event — where it was shot or what it depicts */}
      ...
      {/* Technical — picture properties */}
      <FilterSection icon={Film} label="Technical" columns={3}>
        <FilterField label="File extension">{txt('extension', 'mp4 / mov')}</FilterField>
        <FilterField label="Orientation">{txt('orientation', 'landscape')}</FilterField>
        <FilterField label="Dimension">{txt('dimension', '1920x1080')}</FilterField>
        <FilterField label="Resolution">{txt('resolution', '1080p / 4K')}</FilterField>
        <FilterField label="Duration">{txt('duration', '00:42:11')}</FilterField>
        <FilterField label="Frame rate">{txt('frameRate', '24 / 30 / 60')}</FilterField>
        <FilterField label="Bit depth">{txt('bitDepth', '8 / 10')}</FilterField>
      </FilterSection>

      {/* Codecs / Audio — encoding properties */}
      <FilterSection icon={Settings} label="Codecs / Audio" columns={3}>
        <FilterField label="Video codec">{txt('videoCodec', 'H.264 / H.265')}</FilterField>
        <FilterField label="Audio codec">{txt('audioCodec', 'AAC')}</FilterField>
        <FilterField label="Audio channels">{txt('audioChannels', 'stereo / 5.1')}</FilterField>
        <FilterField label="Overall bit rate">{txt('overallBitRate', '')}</FilterField>
      </FilterSection>
      ...
      {/* People — director / producer / contributor + person shown */}
      <FilterSection icon={Users} label="People" columns={2}>
        <FilterField label="Director / Creator">{txt('creatorArtistDirector', 'name…')}</FilterField>
        <FilterField label="Producer">{txt('producer', 'name…')}</FilterField>
        <FilterField label="Contributor">{txt('contributor', 'name…')}</FilterField>
        <FilterField label="Person shown" hint="contains">{txt('personShownInVideo', 'name…')}</FilterField>
      </FilterSection>
      ...
      <FilterSection icon={Tag} label="Discovery" columns={1}>
        <MultiValueFilter
          ...
          helpText={
            <>
              <span className="font-mono">any</span> matches videos tagged with at least one;{' '}
        ...
        <MultiValueFilter
          ...
          values={filters.colorOfVideo}
          matchMode={filters.colorMatch}
          onValuesChange={(next) => onChange('colorOfVideo', next)}
          onMatchChange={(next) => onChange('colorMatch', next)}
        />
        <MultiValueFilter
          label="Used in"
          placeholder="Type a publication / context and press Enter…"
          values={filters.whereThisVideoUsed}
          matchMode={filters.usageMatch}
          onValuesChange={(next) => onChange('whereThisVideoUsed', next)}
