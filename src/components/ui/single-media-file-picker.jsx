...
/**
 * True single-file picker shared by audio, video, image, and text forms.
 *
 * A standard browser File contains the selected file but not its parent folder.
 * Native runtimes can expose a path; getFileSourcePath handles it when present.
 */
export function SingleMediaFilePicker({
  ...
  const handleFilePicked = (event) => {
    ...
    // The form and upload service receive exactly one File.
    onFileChange(picked)
  }
      ...
      <div className="flex items-center gap-3 rounded-xl border border-border bg-muted/20 p-3.5">
        ...
        <div className="min-w-0 flex-1 space-y-0.5">
          ...
          <p className="text-xs text-muted-foreground">
            {source.path
              ? 'One file selected · available source information was filled below'
              : 'One file selected · Firefox did not provide its parent-folder location'}
          </p>
        </div>
    ...
    <div className="space-y-2.5">
      ...
      >
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Open any folder and select the exact file you need
          </p>
          {acceptedFormats ? (
